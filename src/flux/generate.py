import torch
import yaml, os
from diffusers.pipelines import FluxPipeline, FluxInpaintPipeline
...
def generate_inpaint(    
    ...
    **params: dict,
    ...
    init_image = self.image_processor.preprocess(image_inpaint, height=height, width=width)
...
# 4.Prepare timesteps
    ...
    mask_condition = self.mask_processor.preprocess(mask_image_inpaint, height=height, width=width)
    ...
    with self.progress_bar(total=num_inference_steps) as progress_bar:
        for i, t in enumerate(timesteps):
            ...
            # for 64 channel transformer only. FOR INPAINTING
